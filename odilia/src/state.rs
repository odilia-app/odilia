...
impl ScreenReaderState {
    ...
    pub async fn new() -> eyre::Result<ScreenReaderState> {
        ...
        if !config_path.exists() {
            fs::copy("config.toml", &config_path).expect("Unable to copy default config file.");
        }
    ...
    pub async fn generate_speech_string(
        ...
        let (mut text_selection, start, end) = match select {
            TextSelectionArea::Granular(granular) => {
                acc_text
                    .get_string_at_offset(granular.index, granular.granularity)
                    .await?
            }
            TextSelectionArea::Index(indexed) => {
                (acc_text.get_text(indexed.start, indexed.end).await?, indexed.start, indexed.end)
            }
        };
