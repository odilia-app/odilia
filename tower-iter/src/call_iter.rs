	...
	S: Service<Req, Response = Res, Future = F, Error = E>,
{
	...
	fn poll(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
		let next = match self.as_mut().project() {
			PinnedFsf::ServiceNotReady(ready_fut) => match ready_fut.poll(cx) {
				Poll::Pending => return Poll::Pending,
				Poll::Ready((Err(e), req)) => return Poll::Ready(Err(e)),
				Poll::Ready((Ok(svc), req)) => {
					Self::ServicePending(ServiceCall::new(svc, req))
				}
			},
...
pub trait ServiceExt<I>: Service<I> {
	...
	where
		Self: Sized + Service<I, Response = Res, Error = E, Future = F>,
	{
		FullServiceFut::new(self, input)
	}
}
impl<S, I> ServiceExt<I> for S where S: Service<I> {}
...
impl<S, I> ReadyFut<S, I> {
	pub fn new(s: S) -> Self {
		ReadyFut { inner: Some(s), _marker: PhantomData }
	}
}
	...
	fn poll(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
		...
		match inner.poll_ready(cx) {
			...
			Poll::Ready(Err(e)) => {
				...
				let Some((_owned_svc, owned_input)) =
			...
			Poll::Ready(Ok(())) => {
				// SAFETY: These can only be none if this is polled after returning Poll::Ready.
				let Some((owned_svc, owned_input)) =
					core::mem::take(&mut self.inner)
				else {
					panic!("The items can not be none!");
				};
