mod test_config {
	use crate::config::{
		extract_curly_brace, load, load_file_contents, parse_contents, Error, Hotkey,
		Modifier, ParseError, Prefix,
	};
	use std::fs;
	use std::io::Write;
...
t
    /bin/firefox
		...
		let hotkey_3 =
			Hotkey::new(Some(evdev::Key::KEY_T), vec![], String::from("/bin/firefox"));
