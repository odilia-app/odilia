...
use crate::{
    CacheItem, RelationType, Cache, CacheDriver, ObjectRef, OdiliaError, CacheKey
};
...
impl Relations {
    async fn fill_with<D: CacheDriver>(ors: RelationSet, rt: RelationType, cache: &mut Cache<D>) -> Result<Relations, OdiliaError> {
        let rts: Vec<CacheKey> = ors.get_relations(rt)
            .map(Into::into)
            .collect();
...
impl RelationSet {
  pub fn get_relations(self, rt: RelationType) -> impl Iterator<Item = ObjectRef> {
      self.0.into_iter().filter_map(move |(ty, ci)| if ty == rt { Some(ci) } else { None })
      .flatten()
  }
